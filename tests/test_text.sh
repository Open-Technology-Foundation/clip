...
if assert_success "Copy single file to clipboard" "$CLIP_CMD" "$test_file"; then
  sleep 0.2  # Allow clipboard to settle
...
if assert_success "Copy empty file" "$CLIP_CMD" "$empty_file"; then
  ...
  assert_equal "Empty file produces empty clipboard" "" "$output"
fi
...
if assert_success "Copy large file" "$CLIP_CMD" "$large_file"; then
  sleep 0.3  # Large file needs more time
  output=$("$CLIP_CMD" -p 2>/dev/null || true)
...
# Test: Quiet mode - skipped due to xclip hanging in automated tests
# Uncomment for manual testing
# sleep 0.5  # Allow clipboard system to fully settle before quiet mode test
...
skip_test "Quiet mode test" "Skipped to avoid xclip hangs in automated environment"

# Test: Verbose mode - skipped due to xclip hanging in automated tests
# Uncomment for manual testing
# sleep 0.5  # Allow clipboard system to fully settle before verbose mode test
...
skip_test "Verbose mode test" "Skipped to avoid xclip hangs in automated environment"
